...
Page({
  ...
  onLoad: function(options) {
    ...
    wxRequest({
      url: app.globaldata.site_url + "seckill/detail",
      data: request_data,
      method: 'GET',
      header: {
  ...
  gotopay: function() {
    ...
    wx.navigateTo({
      url: '/pages/miaosha/pay?postpay=' + postpay,
    });
  },
  ...
  set_miaosha: function() {
    ...
    } else if (now < that.data.endtime && now > that.data.starttime) {
      that.setData({
        miaosha_status: 2,
        starttitle: '正在进行',
        endtitle: false,
      })
    } else if (now < that.data.starttime) {
      that.setData({
        miaosha_status: 1,
        starttitle: false,
        endtitle: false,
    ...
    var totalSecond1 = this.data.starttime - Date.parse(new Date()) / 1000; //开始时间倒计时

    var interval1 = setInterval(function() {
      // 秒数
      var second = totalSecond1;
