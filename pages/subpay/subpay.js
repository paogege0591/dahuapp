...
Page({
  ...
  data: {
    ...
    //       pay_type: 1
    //   })
        var that = this, user_id = app.globaldata.uid, money = that.data.money, order_id = that.data.order_id,
            order_sn = that.data.order_sn;
            // console.log(order_id);return;
        wxRequest({
            url: app.globaldata.site_url + "order/pay",
            data: { user_id: user_id, money: money, order_sn: order_sn, order_id: order_id },
            method: 'GET',
            header: {
            ...
            if (res.data.code == 1) {
                var timeStamp = res.data.data.timeStamp.toString(), nonceStr = res.data.data.nonceStr, packages = res.data.data.package, 
                    paySign = res.data.data.paySign, prepay_id = res.data.data.prepay_id;
                wx.requestPayment({
                    'timeStamp': timeStamp,
                    'nonceStr': nonceStr,
                    'package': packages,
                    ...
                    'success': function (res) {
                        wxRequest({
                            url: app.globaldata.site_url + "order/callBackReduce",
    ...
    checkpwd:function(e){
        ...
        wxRequest({
            url: app.globaldata.site_url + "order/remainMoneyPay",
            data: request_data,
            method: 'POST',
            header: {
